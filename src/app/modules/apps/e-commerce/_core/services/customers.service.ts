...
export class CustomersService {
  ...
  findCustomers(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
    return environment.isMockEnabled ? this.findFakeCustomers(queryParams) : this.findRealCustomers(queryParams);
  }
  ...
  findFakeCustomers(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
    ...
    return this.http.get<CustomerModel[]>(API_CUSTOMERS_URL).pipe(mergeMap(res => of(new QueryResultsModel(res))));
  }
  ...
  deleteCustomers(ids: number[] = []): Observable<any> {
    return environment.isMockEnabled ? this.deleteFakeCustomers(ids) : this.deleteRealCustomers(ids);
  }
