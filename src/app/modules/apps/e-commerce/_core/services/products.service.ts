...
export class ProductsService {
  ...
  findProducts(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
    return environment.isMockEnabled ? this.findFakeProducts(queryParams) : this.findRealProducts(queryParams);
  }

  // Fake REST API (Mock)
  // This method emulates server calls
  findFakeProducts(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
    return this.getAllProducts().pipe(mergeMap((res: any) => of(new QueryResultsModel(res))));
  }
  ...
  deleteProducts(ids: number[] = []): Observable<any> {
    return environment.isMockEnabled ? this.deleteFakeProducts(ids) : this.deleteRealProducts(ids);
  }
