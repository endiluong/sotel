...
export class ListStateModel {
  ...
  private updateItem(_item: any) {
    if (!_item.id) {
      const c_index = _.findIndex(this.addedItems, function(o: any) {
      ...
      const u_index = _.findIndex(this.updatedItems, function(o: any) {
  ...
  private deleteItem(_item: any) {
    if (_item.id) {
      const d_index = _.findIndex(this.deletedItems, function(o: any) {
        return o.id === _item.id;
      });
      const u_index = _.findIndex(this.updatedItems, function(o: any) {
        return o.id === _item.id;
