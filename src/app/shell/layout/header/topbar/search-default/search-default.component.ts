import { Component, OnInit, HostBinding, OnDestroy, Input, ElementRef, ChangeDetectionStrategy } from '@angular/core';
...
export class SearchDefaultComponent implements OnInit, OnDestroy {
  ...
  constructor(private layoutConfigService: LayoutConfigService, private el: ElementRef) {
    this.onLayoutConfigUpdated = this.layoutConfigService.onLayoutConfigUpdated$.subscribe(model => {
      ...
      this.headerSearchSkinClass = 'm-list-search--skin-' + objectPath.get(config, 'header.search.dropdown.skin');
